...
while True:
    ...
    driver.get("https://in.bookmyshow.com/buytickets/rrr-hyderabad/movie-hyd-ET00320964-MT/20220325")
    flag = 3
    ...
    given = ['Sudarshan', 'Galleria', 'INOX', 'Devi', 'Shanti']
    ...
    if len(t) > 0:
        talk(" 3D Bookings opened at a given station")
        ...
        print(f"3D booking not opened in given theatres at {datetime.datetime.now()}")

    if len(new) > 0:
        talk("New 3D theatre added :" + new[0])
        hr = int(str(datetime.datetime.now())[11:13])
        min = int(str(datetime.datetime.now())[14:16]) + 2
